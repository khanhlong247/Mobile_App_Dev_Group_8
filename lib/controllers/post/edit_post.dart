  ...
  Future<List<String>> uploadNewImages() async {
    ...
    if (kIsWeb) {
      for (var i = 0; i < newImagesBytes.length; i++) {
        ...
        request.files.add(http.MultipartFile.fromBytes('file', newImagesBytes[i],
      ...
      for (var i = 0; i < newImages.length; i++) {
        ...
        request.fields['upload_preset'] = 'your_unsigned_upload_preset';
        request.fields['public_id'] = 'post_${date}_$i';
        request.files.add(http.MultipartFile.fromBytes('file', bytes, filename: 'upload_$i.jpg'));
        var response = await request.send();
        if (response.statusCode == 200) {
          var resStream = await response.stream.bytesToString();
          var jsonResponse = json.decode(resStream);
            ...
            child: Column(
              children: [
                      ...
                      return Stack(
                        children: [
                          Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: kIsWeb
                                ? Image.memory(newImagesBytes[index],
                                    width: 150, height: 150, fit: BoxFit.cover)
                                : Image.file(newImages[index],
                                    width: 150, height: 150, fit: BoxFit.cover),
                          ),
                          Positioned(
                            right: 0,
                ...
                ElevatedButton(
                  ...
                  child: const Text("Add New Images", style: TextStyle(color: Colors.white)),
                ),
